document.addEventListener('DOMContentLoaded', () => {
    ...
    const dotsContainer = stagesWrapper.querySelector('.stages-dots');
      ...
      //Отключение стрелки на последних слайдах
    const updateArrows = () => {
        // Проверка, находится ли пользователь на первом слайде
        if (currentSlide === 0) {
    ...
    // Инициализация точки-индикатора
    stages.forEach((_, index) => {
        updateArrows(); // Обновление стрелки
    ...
    // Переключение слайдов с помощью стрелки
    ...
    // Обновляемk что бы был виден первый слайд
    updateSlidePosition();
    });
parseInt
